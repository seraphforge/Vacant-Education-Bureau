...
- 作品簡報：**https://hackmd.io/@wu-pin-jui/H18p3uQtGe**
...
- [x] 財報法遵分析整合（決算書 113 年度，10 所非營利幼兒園）／[ ] Google Map 評論
- [x] AI 風險指數分析（四維度加權：財務法遵／家長回報／輿情關注／裁罰紀錄；缺資料的維度不計分）
- [ ] 家長回報系統
...
- **同名園所一律標為待人工確認。** 「可歸屬本園」與「待人工確認」在畫面上是兩張分開的表；
  只有前者會計入分數。
- **掃描完成才更新風險指數。** worker 把 job 標成 done 之後才重算風險，避免用一半的結果算分；輿情從未掃描的園所，該維度是「缺資料不計分」而不是 0 分。
