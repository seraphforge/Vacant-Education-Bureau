...
- `--cash-flow-only` / `--cash-flow-pages`：只抽取現金流量表（預設第 9 頁），輸出項目與本年度／前一年度金額，存成 `<前綴>_現金流量表.csv`。
- `--appendix2-only` / `--appendix2-pages`：只抽取附表二「經費流用及勻支檢查表」（預設第 25 頁），輸出項目、預算數、決算數、差異數、差異率、預決算檢查結果，存成 `<前綴>_附表二_經費流用及勻支檢查表.csv`。
- `--appendix4-only` / `--appendix4-pages`：只抽取附表四「財產清冊」（預設第 28~32 頁，逐頁抽取後合併、分類向下填充），存成 `<前綴>_附表四_財產清冊.csv`。
