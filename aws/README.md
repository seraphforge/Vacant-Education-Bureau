...
- **八項指標的等級**：GREEN / YELLOW / RED，對應 CSV 的「等級分數」1 / 2 / 3，
  ...
  前端不用自己過濾
- **每項指標的風險方向**：`過高`（HIGH）／`過低`（LOW）／`異常變化`（SHIFT，
  相對自身歷史突然改變，不分升降），來源是 CSV 的 `{指標}_風險方向`，
  可用 `+` 串接（`HIGH+SHIFT`）。**等級只說「有多異常」，方向才說「往哪個方向異常」**——
  每生人事費判 RED 可能是花太多也可能是低到不合理，處置完全不同，所以畫面兩者都顯示。
  中文與圖示定在 `finance.py` 的 `DIRECTION_LABELS` / `DIRECTION_ICONS`，前端不自己翻
