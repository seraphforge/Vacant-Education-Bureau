...
- [x] Todo：確認雷達圖各項指標名稱與資料來源欄位（規格未列出具體指標項目，待補）
  - 已解決：四個維度由後端定案（API_SPEC §4.8）：**財務法遵、家長回報、輿情關注、裁罰紀錄**
    （原本的「資料完整度」已移除 —— 它衡量的是我們的資料收集狀況，不是園所的風險）。
    分數為正式演算法（`risk-v1`）的加權平均；缺資料的維度不計分並在畫面標示。

#### Tab 2：財報

- [x] 以表格呈現財報資料
  - 已完成：API_SPEC §4.9。顯示**法遵風險指數**、整體風險等級、八項指標的等級
    （1 正常 / 2 注意 / 3 警示，另外把等級 ≥ 2 的挑成「需注意的指標」）與主要財務數字。
    只有 10 所非營利幼兒園有 113 年度決算書分析，其餘顯示「尚無決算書分析資料」。
...
- [x] 風險評估 §4.8 與財報 §4.9 **已改呼叫真實 API**，前端已無任何 mock
